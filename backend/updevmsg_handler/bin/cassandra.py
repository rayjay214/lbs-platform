...
class CassandraOp():
    ...
    def insert_alarm(self, alarm):
        send_tm = arrow.now().format('YYYY-MM-DD HH:mm:ss')
        receive_tm = arrow.get(alarm.alarmtime).format('YYYY-MM-DD HH:mm:ss')
        sql = '''insert into alarm(dev_id, course, lat, lng, receive_time, 
            ...
            alarm.lng, receive_tm, send_tm, alarm.speed, alarm.status, alarm.alarmtype)
        try:
